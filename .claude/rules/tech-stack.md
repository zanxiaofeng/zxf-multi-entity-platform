...
- RestClient / RestTemplate（下游 HTTP 客户端，需 `spring-boot-starter-restclient`；RestClient 为新模块首选，RestTemplate 处于维护模式）
...
**Test starter 配套规则：** `spring-boot-starter-<tech>-test`（如 `spring-boot-starter-webmvc-test`、`spring-boot-starter-restclient-test`）只提供该技术的测试自动配置切片（MockMvc 自动配置等），**不含** JUnit / AssertJ / Mockito / Awaitility / json-path——断言与 mock 库仍由 `spring-boot-starter-test` 提供，两者都需显式声明（SB 4.1.0 实测 `dependency:tree` 验证）。
...
- RestClient with 3s connect / 5s read timeout
