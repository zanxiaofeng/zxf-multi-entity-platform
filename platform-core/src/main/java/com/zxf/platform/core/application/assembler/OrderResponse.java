...
public record OrderResponse(String id, String item, int quantity, Money price, OrderStatus status) {
    ...
    public static OrderResponse from(Order order) {
        ...
        return new OrderResponse(order.id().value(), order.item(), order.quantity(), order.price(), order.status());
    }
}
