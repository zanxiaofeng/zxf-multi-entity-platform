-- Flowable 8.0.0 ACT_* 引擎表（文档 7.3② / 规则 9）：
--   flowable.database-schema-update=false，引擎表结构统一归 Flyway 管（common 目录，属核心层变更）。
--   本脚本提取自官方 jar 内建 H2 DDL（官方只发布 Liquibase changelog，Flyway 脚本团队自维护）：
--     flowable-engine-common: org/flowable/common/db/create/flowable.h2.create.common.sql
--     flowable-engine:        org/flowable/db/create/flowable.h2.create.engine.sql
--     flowable-engine:        org/flowable/db/create/flowable.h2.create.history.sql
--     flowable-event-registry: org/flowable/eventregistry/db/create/flowable.h2.create.eventregistry.sql
--     flowable-idm-engine:    org/flowable/idm/db/create/flowable.h2.create.identity.sql
--   注意：IDM（ACT_ID_*）表易被漏掉——它不在流程引擎 jar 里，但引擎启动会强制校验
--   ACT_ID_PROPERTY（缺失时报误导性的 "db version is 5.99.0.0"）。
--   每次升级 Flowable 版本 = 人工核对 ACT_* 差异并补齐本目录脚本（持续运维成本，文档 7.3②）。
--   生产换 Oracle 时重新从对应 jar 提取 oracle 方言脚本。

-- ============ common（ACT_GE_* / 任务 / 变量 / Job / 事件订阅 / 批次） ============
...
-- ============ engine（ACT_RE_* / ACT_RU_EXECUTION / ACT_EVT_LOG / ACT_RU_ACTINST） ============
...
-- ============ history（ACT_HI_PROCINST / ACTINST / DETAIL / COMMENT / ATTACHMENT） ============
...
-- ============ eventregistry（FLW_EVENT_* / FLW_CHANNEL_*） ============
...
-- ============ identity（ACT_ID_*：Flowable 8 进程引擎内嵌 IDM 服务，启动时强制校验 ACT_ID_PROPERTY，缺失报 5.99.0.0 版本错） ============
